...
while loop :
    ...
    if loop:
        ...
        the_k = float(the_k)
    if loop:
        data_table = [["Method/Param",    "Data Size (After)", "Data Training Size", "Akurasi", "F-Measure","Precision","Recall","TP","FP","TN","FN"]]#menampung hasil hitungan
        ...
        scores = cross_val_score(clf , X_test,y_test, cv=3,scoring='accuracy') #10fold cross validation
        f1_macro = cross_val_score(clf , X_test,y_test, cv=3,scoring='f1_macro') #10fold cross validation
        data_table.append(['ADASYN',str(X_rknn.shape),str(X_train.shape), "%0.2f" % (akurasi),"%0.2f" % (fm),"%0.2f" % (presisi),"%0.2f" % (recall) , tp, fp , tn ,fn ])
